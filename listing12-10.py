...
def drop_object(old_y, old_x):
    ...
    if room_map[old_y][old_x] in [0, 2, 39]:
        ...
        time.sleep(0.5)
    else: # Only happens if there’s already a prop here.
...
def examine_object():
    ...
    for prop_number, details in props.items():
        ...
        if details[0] == current_room: # If prop in the room.
            # If prop hidden (= at player’s location but not on map)
...
def use_object():
    ...
    if item_carrying == 70 or item_player_is_on == 70:
        ...
        props[25][0] = 0 # Soor from RM32 to engineering bay.
...
def game_completion_sequence():
    ...
    box = Rect((0, (top_left_y - 3)), (800, 390))
    screen.surface.set_clip(box)
...
def close_door(closing_door_number):
    ...
    # If player in same row as a door, must be in open doorway.
...
def door_in_room_26():
    ...
    if ((player_y == 8 and player_x == 2)
        or props[63] == [26, 8, 2]) and props[21][0] == 26:
    ...
    if ((player_y != 8 or player_x != 2)
        and props[63] != [26, 8, 2]) and airlock_door_frame > 0:
        if airlock_door_frame == 5:
...
def deplete_energy(penalty):
    ...
    if game_over:
        return # Don’t sap energy when player already dead.
...
def hazard_move():
    ...
    for hazard in current_room_hazards_list:
        ...
        if ((hazard_y == player_y and hazard_x == player_x)
            or (hazard_y == from_player_y and hazard_x == from_player_x
                and player_frame > 0)):
            sounds.ouch.play()
            deplete_energy(10)
...
clock.schedule_unique(alarm, 30)
